...
function SwampMiddle ({fullLine, setmiddleGame, setfullLine, setchangeImage }) {
    ...
    const charname = SwampLine.middle[idx].char
    const line1 = SwampLine.middle[idx].line1
    const line2 = SwampLine.middle[idx].line2

    function checkDialog () {
        if (idx + 1 === SwampLine.middle.length) {
            setmiddleGame(true)
            setchangeImage(true)
            return
        } 
    ...
    return (
        <View>
            ...
            onPress={() => { setmiddleGame(true), setchangeImage(true) } }>건너뛰기</Animated.Text>
